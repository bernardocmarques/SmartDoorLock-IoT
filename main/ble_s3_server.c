...
static const uint8_t spp_adv_data[23] = {
        ...
        0x0F,0x09, 'S', 'm', 'a', 'r', 't', 'L', 'o', 'c', 'k', 'B', 'L','E', 'S','3'
};
...
void send_data_ble_s3(char* data) {
    ...
    if(len <= (spp_mtu_size - 3)){
        ...
        if(data_frag == NULL){
            ESP_LOGE(BLE_S3_TAG, "%s malloc.2 failed\n", __func__);
            free(data);
            return;
        }
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
    esp_err_t err;
    if (VERBOSE_LEVEL >= 3) ESP_LOGI(BLE_S3_TAG, "GAP_EVT, event %d\n", event);
    ...
    switch (event) {
        ...
        case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
            ...
            if((err = param->adv_start_cmpl.status) != ESP_BT_STATUS_SUCCESS) {
                ESP_LOGE(BLE_S3_TAG, "Advertising start failed: %s\n", esp_err_to_name(err));
            }
            break;
        default:
            break;
...
static void process_normal_data(char* data) {
    ESP_LOGE(BLE_S3_TAG, "DATA ----> %s", data);
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT: // When register application id
            esp_ble_gap_set_device_name(SAMPLE_DEVICE_NAME);
            esp_ble_gap_config_adv_data_raw((uint8_t *)spp_adv_data, sizeof(spp_adv_data));
            esp_ble_gatts_create_attr_tab(spp_gatt_db, gatts_if, SPP_IDX_NB, SPP_SVC_INST_ID);
            break;
        case ESP_GATTS_READ_EVT: // When gatt client request read operation
            res = find_char_and_desr_index(p_data->read.handle);
            if(res == SPP_IDX_SPP_STATUS_VAL){
                //TODO:client read the status characteristic
            }
            break;
        case ESP_GATTS_WRITE_EVT: // When gatt client request write operation
        ...
        case ESP_GATTS_EXEC_WRITE_EVT: // When gatt client request execute write
            if (VERBOSE_LEVEL >= 2) ESP_LOGI(BLE_S3_TAG, "ESP_GATTS_EXEC_WRITE_EVT\n");
            break;
        case ESP_GATTS_MTU_EVT: // When set mtu complete
            spp_mtu_size = p_data->mtu.mtu;
            break;
        case ESP_GATTS_CONF_EVT: // When receive confirm
            break;
        case ESP_GATTS_UNREG_EVT: // When unregister application id
            break;
        case ESP_GATTS_DELETE_EVT: // When delete service complete
            break;
        case ESP_GATTS_START_EVT: // When start service complete
            break;
        case ESP_GATTS_STOP_EVT: // When stop service complete
            break;
        case ESP_GATTS_CONNECT_EVT: // When gatt client connect
            ...
            break;
        case ESP_GATTS_DISCONNECT_EVT: // When gatt client disconnect
            is_connected = false;
            enable_data_ntf = false;
            esp_ble_gap_start_advertising(&spp_adv_params);
            ...
            disconnect_lock();
            break;
        case ESP_GATTS_OPEN_EVT: // When connect to peer
            break;
        case ESP_GATTS_CANCEL_OPEN_EVT: // When disconnect from peer
            break;
        case ESP_GATTS_CLOSE_EVT: // When gatt server close
            break;
        case ESP_GATTS_LISTEN_EVT: // When gatt listen to be connected
            break;
        case ESP_GATTS_CONGEST_EVT: // When congest happen
            break;
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: // When gatt create table complete
            if (VERBOSE_LEVEL >= 2) ESP_LOGI(BLE_S3_TAG, "The number handle =%x\n", param->add_attr_tab.num_handle);
            if (param->add_attr_tab.status != ESP_GATT_OK){
                ESP_LOGE(BLE_S3_TAG, "Create attribute table failed, error code=0x%x", param->add_attr_tab.status);
            }
            else if (param->add_attr_tab.num_handle != SPP_IDX_NB){
                ESP_LOGE(BLE_S3_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to HRS_IDX_NB(%d)", param->add_attr_tab.num_handle, SPP_IDX_NB);
            }
            else {
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    if (VERBOSE_LEVEL >= 2) ESP_LOGI(BLE_S3_TAG, "EVT %d, gatts if %d\n", event, gatts_if);
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGE(BLE_S3_TAG, "Reg app failed, app_id %04x, status %d\n", param->reg.app_id, param->reg.status);
            return;
        }
    }
...
void ble_s3_main(void) {
    ...
    if (ret) {
        ESP_LOGE(BLE_S3_TAG, "%s enable controller failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }
    ...
    ESP_LOGI(BLE_S3_TAG, "%s init bluetooth\n", __func__);
    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(BLE_S3_TAG, "%s init bluetooth failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }
    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(BLE_S3_TAG, "%s enable bluetooth failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }
