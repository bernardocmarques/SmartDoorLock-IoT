        ...
        if (len < 0) {
            ...
            if (key_256 == NULL) {
                if (retrieve_session_credentials(rx_buffer)) {
                    ...
                    response = "RAC seed"; //TODO make new seed
                    // ESP_LOGE(TAG, "Time to get credentials: %ld", (long) ccomp_timer_stop()); //FIXME remove timer
                } else {
                    key_256 = NULL;
...
static void tcp_server_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // ccomp_timer_start(); //FIXME remove timers
...
void app_main(void) {
    ...
    // authorization* auth = malloc(authorization);
